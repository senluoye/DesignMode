...
又叫快照模式，在不破坏封装性的前提下，捕获一个对象的**内部状态**，并在该**对象之外**保存这个状态，以便以后当需要时能将该对象**恢复到原先保存的状态**。

### 20.2 结构

备忘录模式的主要角色如下：

- 发起人（Originator）角色：记录当前时刻的内部状态信息，提供**创建备忘录**和**恢复备忘录**数据的功能，实现其他业务功能，它可以访问备忘录里的所有信息。
- 备忘录（Memento）角色：负责存储**发起人的内部状态**，在需要的时候提供这些内部状态给发起人。
...
![20220512143132](https://raw.githubusercontent.com/senluoye/BadGallery/master/image/20220512143132.png)
...
![20220512143147](https://raw.githubusercontent.com/senluoye/BadGallery/master/image/20220512143147.png)
...
> 要是需要让类 A 持有类 B 的实例，但是又不能让类 A 操作类 B，那么就可以借鉴这里的写法，让类 B 实现一个没有任何方法的"标识接口" C，然后让类 A 持有该接口 C 的实例。
...
- 简化了发起人类。发起人**不需要管理和保存其内部状态的各个备份**，所有状态信息都保存在备忘录中，并由管理者进行管理，这符合单一职责原则。
